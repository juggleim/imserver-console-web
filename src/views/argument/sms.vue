...
import { RESPONSE, SMS_CHANNELS, R3D_USE_TYPE } from '../../common/enum';
...
let channels = utils.clone(SMS_CHANNELS);
let globalData = {};
let state = reactive({
  list: [],
  current: channels[0],
...
function create(conf, _params){
  R3d.setSms({ app_key, conf }).then((result) => {
    let { code, data } = result;
    if(!utils.isEqual(code, RESPONSE.SUCCESS)){
      return context.proxy.$toast({ icon: 'error', text: t('smsConfig.feedback.createFailed', { code }) });
    }
    context.proxy.$toast({ icon: 'success', text: t('smsConfig.feedback.success') });
    updateList(_params);
    onShowDialog(false);
  });
}
...
function updateConf(conf){
  R3d.setSms({ app_key, conf }).then((result) => {
    let { code, data } = result;
    if(!utils.isEqual(code, RESPONSE.SUCCESS)){
      return context.proxy.$toast({ icon: 'error', text: t('smsConfig.feedback.updateFailed', { code }) });
    }
    context.proxy.$toast({ icon: 'success', text: t('smsConfig.feedback.success') });
    onShowDialog(false);
  });
...
R3d.getSms({ app_key }).then((result) => {
  let { code, data } = result;
  if(!utils.isEqual(code, RESPONSE.SUCCESS)){
    return context.proxy.$toast({ icon: 'error', text: t('smsConfig.feedback.fetchFailed', { code }) });
  }
...
<template>
  <PageSection title-key="menu.app.smsSettings" body-class="cim-r3-container-sms">
      ...
      <li class="cim-r3-item" v-for="(item, index) in state.list">
        <div class="cim-r3-item-header">
          ...
          <div class="cim-r3-header-status">
            <span class="life-status" v-if="item.isUsed">{{ t('smsConfig.status.enabled') }}</span>
          </div>
        </div>
        <ul class="cim-r3-item-contents">
          <li class="cim-rtc-item-content" v-for="child in item.children">
            ...
            <div class="value unset"  v-if="(child.type == 'text' || child.type == 'number') && !child.value">{{ t('smsConfig.status.unset') }}</div>
          </li>
        </ul>
        <ul class="cim-r3-item-tools">
          <li class="cim-r3-item-tool cicon wr-update" @click="onShowDialog(true, item)">{{ t('smsConfig.action.settings') }}</li>
    ...
    <TranslateDialog
      ...
      :title="t('smsConfig.dialog.title')"
      :custom="t('smsConfig.dialog.channel')"
